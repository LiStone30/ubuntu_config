...
```bash
# 假设你的代理地址是 http://192.168.1.100:7890（替换为实际地址）
docker build \
...
- **关键说明**：
  ...
  ```bash
# 假设你的代理地址是 http://127.0.0.1:7890（替换为实际地址）
docker build \
...
docker buildx create \
  --use \
  --name mybuilder \
  --buildkitd-flags '--opt env.HTTP_PROXY=http://192.168.1.100:7890' \
  --buildkitd-flags '--opt env.HTTPS_PROXY=http://192.168.1.100:7890'
...
RUN git config --global http.proxy http://192.168.1.100:7890 && \
...
1. **容器访问主机代理的地址**：
   ...
   - 也可以直接用主机的内网 IP（如 `192.168.1.100`）。
2. **代理服务需允许容器访问**：
